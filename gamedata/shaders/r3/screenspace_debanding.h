...
static const float2 dband_dir_Offsets[4] = {float2(1, 1), // XOX
                                            float2(-1, -1), // O0O
                                            float2(-1, 1), // XOX
                                            float2(1, -1)};
...
void dband_process(inout float3 color, float2 tc, float noise, float2 dir, float loop_step)
{
    ...
    [unroll(4)] for (int taps = 0; taps <= 4; taps++)
    {
        // Sample pixel
...
float3 ssfx_debanding(float3 color, float2 tc)
{
    ...
    [unroll(G_DEBANDING_QUALITY)] for (int loop_cnt = 1; loop_cnt <= G_DEBANDING_QUALITY; loop_cnt++)
    {
